...
function output () {
	...
	#array_contains PRINT_VALUES "status" && prints+=("Connected")
        #array_contains PRINT_VALUES "contime" && prints+=(`$status | grep "Connection time": | awk '{ print $3 }'`)
        #size=${#prints[@]}
        out="%{F$COLOR_CONNECTED}$ICON_CONNECTED %{F$COLOR_DISCONNECTED}"
        ts=`date +%s`
        ...
        out="%{F$COLOR_DISCONNECTED}$ICON_DISCONNECTED %{F$COLOR_CONNECTED}%{F-}"
    fi
    echo "%{A1:$DIR/protonvpn.sh -t:}$out%{A}"
}
while getopts 'ot' c
do
    case $c in
